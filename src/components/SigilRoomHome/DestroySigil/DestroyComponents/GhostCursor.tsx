...
// FIX: Moved static shader strings outside the component to avoid
// them being "new" on every render (which would cause dep array issues)
...
const GhostCursor = ({
  ...
  // FIX: Initialize to 0, then set in useEffect to avoid calling impure
  // functions (performance.now / Date.now) during render
  ...
  // FIX: useMemo so FilmGrainShader is stable across renders and safe to
  // include in the main useEffect dependency array
  ...
  useEffect(() => {
    ...
    // FIX: mixBlendMode applied here on setup; the main effect re-runs when
    // mixBlendMode changes (it's in the dep array), so no separate effect needed
    if (mixBlendMode) {
    ...
    const resize = () => {
      ...
      if (material.uniforms['iResolution']) {
        (
          material.uniforms['iResolution'].value as THREE.Vector3
        ).set(wpx, hpx, 1);
      }
    ...
    const animate = () => {
      ...
      if (pointerActiveRef.current) {
        velocityRef.current.set(
          currentMouseRef.current.x -
            (mat.uniforms['iMouse']!.value as THREE.Vector2).x,
          currentMouseRef.current.y -
            (mat.uniforms['iMouse']!.value as THREE.Vector2).y
        );
        (mat.uniforms['iMouse']!.value as THREE.Vector2).copy(
        ...
        if (velocityRef.current.lengthSq() > 1e-6) {
          (mat.uniforms['iMouse']!.value as THREE.Vector2).add(
            velocityRef.current
          );
        }
      ...
      trailBufRef.current[headRef.current]!.copy(
        mat.uniforms['iMouse']!.value as THREE.Vector2
      );
    ...
    mixBlendMode,
    edgeIntensity,
    // FIX: Added missing deps — maxDevicePixelRatio, FilmGrainShader, UnpremultiplyPass
    maxDevicePixelRatio,
    FilmGrainShader,
    UnpremultiplyPass,
  ]);
  ...
  useEffect(() => {
    if (materialRef.current) {
      ...
      (
        materialRef.current.uniforms['iBaseColor']!.value as THREE.Vector3
      ).set(c.r, c.g, c.b);
    }
  }, [color]);
  ...
  // FIX: Removed the standalone mixBlendMode useEffect that was causing the
  // react-hooks/immutability error (modifying rendererRef inside a separate
  // effect). The main effect already applies mixBlendMode on setup and
  // re-runs when mixBlendMode changes via the dependency array.
